...
<template>
  <div class="mt-5">
    <div>
      <v-row justify="end"
             v-if="[$keys.ACCOUNT_SUPER_ADMIN, $keys.ACCOUNT_BUSINESS_DEVELOPER, $keys.ACCOUNT_ADMIN].includes($store.state.user.account_type)">
        <v-col class="" md="6">
          <div class="text-right">
            <span class="fc-primary cursor-pointer" @click="$router.push({name:'add_subscription'})">
    ...
    <v-row class="mb-6">
      <v-col class="text-right" cols="12" md="4">
        <div v-for="(status,i) in subscription_status_options" :key="i"
    ...
    <div>
      ...
      >
        ...
        <template v-slot:[`item.action`]="{ item }">
          ...
          <a class="d-inline"
             :href="$router.resolve({name:'student_subscription_details',params:{std_subscription_id:encrypt(item.id)}}).href">
