<template>
  <div>
    <v-form
      ref="subscription_plan_form"
      lazy-validation
      @submit.prevent="addSubscriptionPlan()"
    >
...
export default {
  ...
  data() {
    return {
      ...
      plan_type_list: [
        this.$keys.BASIC_PLAN,
        this.$keys.STANDARD_PLAN,
        this.$keys.PREMIUM_PLAN,
      ],
      grade_list: [],
      garde_loading: false,
      city_search_query: "",
  ...
  methods: {
    ...
    getSubscriptionPlanDetails() {
      ...
      const successHandler = (response) => {
        if (response.data.success) {
          ...
          self.form.city = { id: response.data.city_id, name: response.data.city_name };
        }
      };
    ...
    addSubscriptionPlan() {
      const self = this;
      if (this.$refs.subscription_plan_form.validate() == false) return false;
      ...
      form.append("city_id", self.form.city.id);
      ...
      const successHandler = (response) => {
        if (response.data.success) this.$router.push({ name: "subscription_list" });
      };
      const finallyHandler = () => {
