    ...
    <div>
      ...
      >
        ...
        <template v-slot:item.lead_type="{ item }">
          <span :class="item.lead_type.toLowerCase()+'_lead text-center'" class="px-3 py-1">
            {{ item.lead_type }}
          </span>
        </template>
        <!--    lead status     -->
        <template v-slot:item.lead_status="{ item }">
          <span :class="item.lead_status.toLowerCase()+'_lead text-center'" class="px-3 py-1">
            {{ item.lead_status }}
          </span>
        </template>
        ...
        <template v-slot:item.action="{ item }">
          ...
          <v-menu offset-y>
            <template v-slot:activator="{ on, attrs }">
              <v-btn class="ml-2" color="primary" outlined fab x-small v-bind="attrs" v-on="on">
            ...
            <v-list>
              ...
              >
                <v-list-item-title @click="openActionDialog(action.value,item)" class="cursor-pointer"
  ...
  methods: {
    ...
    openActionDialog(action, item) {
      if (action == this.$keys.SCHEDULE_DEMO && item.student_table_id){
        this.$router.push({name: 'schedule_demo_class', query: {student_table_id: this.encrypt(item.student_table_id)}})
      }
      else {
