...
<template>
  <div>
      ...
      <v-row justify="end">
        <v-col class="" md="6">
          <div class="text-right">
            <span class="fc-primary cursor-pointer" @click="$router.push({ name: 'add_student' })">
              {{ $lang.ADD_STUDENTS }}
    ...
    <v-row class="mb-6">
      <v-col cols="12" md="6">
        <v-text-field v-model="search_query" :placeholder="$lang.STUDENT_SEARCH" append-icon="mdi-magnify" dense
          hide-details outlined rounded single-line @keyup="getStudentList"></v-text-field>
      </v-col>
      ...
      <v-col class="text-right mt-4" cols="12" md="4" v-if="$project.NAME== $keys.TF">
        <span @click="downloadExcel" :style="{
          opacity: student_filter_data.filter_date_from && student_filter_data.filter_date_to  ? 1 : 0.4,
        }">
          <ExportExcelBtn />
        </span>
      </v-col>
    ...
    <div>
      <v-data-table :disable-sort="true" :headers="headers" :hide-default-footer="true" :items="student_list"
        ...
        <template v-slot:[`item.name`]="{ item }">
          <AccountInfoCard :type="$keys.ACCOUNT_STUDENT"
            :account_details="{ student_table_id: item.id, name: item.name, mobile: item.mobile, email: item.email, gender: item.gender, }" />
        </template>
        <!-- swith btn -->
        <template v-slot:[`item.action1`]="{ item }">
          <v-switch v-model="item.is_active" :class="item.is_active ? 'active-toggle-btn' : 'deactive-toggle-btn'"
            :ripple="false" class="custom-toggle-btn" color="error" dense inset style="display:inline-flex;width: 50px"
            @click="openToggleAccountDialog(item)"></v-switch>
        </template>
        ...
        <template v-slot:[`item.action`]="{ item }">
          <a class="d-inline"
            :href="$router.resolve({ name: 'edit_student_profile', query: { student_table_id: encrypt(item.id) } }).href">
            <TableEditBtn></TableEditBtn>
          </a>
          <a class="d-inline"
            :href="$router.resolve({ name: 'student_profile', query: { student_table_id: encrypt(item.id) } }).href">
      ...
      <v-pagination v-model="page_number" :length="total_page_count" :total-visible="7" class="custom-pagination"
        @input="getStudentList"></v-pagination>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Profile Status", value: "profile_status", align: "center",width:"120px" },
  ...
  methods: {
    ...
    openToggleAccountDialog(item) {
      ...
      this.dialog.msg = !item.is_active ? "Are you sure want to <b>Block</b>?\n <b>" + item.name + "</b>" : "Are you sure want to<b> Unblock</b>? \n <b>" + item.name + "</b>";
    ...
    downloadExcel() {
      if (this.student_filter_data.filter_date_from && this.student_filter_data.filter_date_to) {
        let URL =
