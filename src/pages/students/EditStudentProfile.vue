      ...
      <v-row :class="{ 'px-0': $vuetify.breakpoint.smAndDown, 'px-0': $vuetify.breakpoint.mdAndUp }" class="ma-0">
        <v-col :class="{ 'px-0': $vuetify.breakpoint.smAndDown, 'px-4': $vuetify.breakpoint.mdAndUp }" cols="12" md="8">
          ...
          <v-row :class="{ 'px-0': $vuetify.breakpoint.smAndDown, 'px-0': $vuetify.breakpoint.mdAndUp }" class="ma-0">
            <v-col class="py-0" cols="12" md="6">
              <div class="form-filed-wrapper">
                <label class="label-text-1">{{ $lang.STUDENT_NAME }} <sup class="error--text">*</sup></label>
                <v-text-field v-model="form.name" :placeholder="$lang.STUDENT_NAME"
                  :rules="[$rules.REQUIRED_FIELD($lang.STUDENT_NAME)]" class="mt-1" dense maxlength="80" outlined
            ...
            <v-col class="py-0" cols="12" md="6">
              <div class="form-filed-wrapper">
                <label class="label-text-1"> Father's {{ $lang.MOBILE_NUMBER }} <sup class="error--text">*</sup></label>
                <v-text-field v-model="form.father_contact_number" :placeholder="$lang.MOBILE_NUMBER"
                  :rules="$rules.MOBILE" class="mt-1" dense maxlength="10" outlined single-line>
            ...
            <v-col class="py-0" cols="12" md="6">
              <div class="form-filed-wrapper">
                ...
                <v-text-field v-model="form.guardian_contact_number" :placeholder="$lang.WA_MOBILE_NUMBER"
                  :rules="$rules.MOBILE" class="mt-1" dense maxlength="10" outlined single-line>
                  <template slot="prepend-inner">
                    <span class="mt-1">+91</span>
                  </template>
            ...
            <v-col class="py-0" cols="12" md="6">
              <div class="form-filed-wrapper">
                ...
                <v-text-field v-model="form.pin_code" :placeholder="$lang.PIN_CODE"
                  :rules="[$rules.PIN_CODE($lang.PIN_CODE)]" class="mt-1" dense maxlength="6" outlined single-line
                  @change="getPincodeData(form.pin_code)" :loading="pin_loading"></v-text-field>
              </div>
              <div class="form-filed-wrapper">
                <label class="label-text-1"> {{ $lang.CITY }} <sup class="error--text">*</sup></label>
                <v-text-field v-model="city" :placeholder="$lang.CITY" :rules="[$rules.REQUIRED_FIELD($lang.CITY)]"
                  class="mt-1" dense maxlength="80" outlined single-line readonly></v-text-field>
              </div>
            </v-col>
          ...
          <v-row :class="{ 'px-0': $vuetify.breakpoint.smAndDown, 'px-0': $vuetify.breakpoint.mdAndUp }" class="ma-0">
            <v-col class="py-0" cols="12" md="6">
              <div class="form-filed-wrapper">
                <label class="label-text-1"> {{ $lang.SCHOOL_NAME }} <sup class="error--text">*</sup></label>
                <v-text-field v-model="form.school_name" :placeholder="$lang.SCHOOL_NAME"
                  :rules="[$rules.REQUIRED_FIELD($lang.SCHOOL_NAME)]" class="mt-1" dense maxlength="250" outlined
                  single-line></v-text-field>
              </div>
            </v-col>
            <v-col class="py-0" cols="12" md="6">
              <div class="form-filed-wrapper">
                <label class="label-text-1"> {{ $lang.EDUCATION_BOARD }} <sup class="error--text">*</sup></label>
                <v-text-field v-model="form.education_board" :placeholder="$lang.EDUCATION_BOARD"
                  :rules="[$rules.REQUIRED_FIELD($lang.EDUCATION_BOARD)]" class="mt-1" dense maxlength="250" outlined
                  single-line></v-text-field>
              </div>
            </v-col>
          </v-row>
          <v-row :class="{ 'px-0': $vuetify.breakpoint.smAndDown, 'px-0': $vuetify.breakpoint.mdAndUp }" class="ma-0">
            <v-col class="py-0" cols="12" md="6">
              <div class="form-filed-wrapper">
                <label class="label-text-1"> {{ $lang.CLASS }} <sup class="error--text">*</sup></label>
                <v-select v-model="form.class_id" :items="class_list" :placeholder="$lang.CLASS"
                  :rules="[$rules.REQUIRED_FIELD($lang.CLASS)]" class="mt-1" dense item-text="class_name"
                  item-value="id" maxlength="80" outlined single-line></v-select>
              </div>

              <div class="form-filed-wrapper">
                <label class="label-text-1"> {{ $lang.SUBJECTS }} <sup class="error--text">*</sup></label>
                <v-select v-model="subject_id_list" :items="subject_list" :placeholder="$lang.SUBJECTS"
                  :rules="[$rules.REQUIRED_LIST_FIELD($lang.SUBJECTS)]" class="mt-1" dense item-text="subject_name"
                  item-value="id" maxlength="80" multiple outlined single-line @focus="getSubjectList"></v-select>
              </div>

              <div class="form-filed-wrapper">
                <label class="label-text-1"> {{ $lang.TUTOR_PREFERENCE }} <sup class="error--text">*</sup></label>
                <v-select v-model="form.tutor_preference" :items="tutor_preference_options"
                  :placeholder="$lang.TUTOR_PREFERENCE" :rules="[$rules.REQUIRED_FIELD($lang.TUTOR_PREFERENCE)]"
                  class="mt-1" dense maxlength="80" outlined single-line></v-select>
              </div>
            </v-col>
            <v-col class="py-0" cols="12" md="6">
              <div class="form-filed-wrapper">
                <label class="label-text-1"> {{ $lang.SPECIAL_CONCERN }} </label>
                <v-textarea v-model="form.special_concern" :placeholder="$lang.SPECIAL_CONCERN" class="mt-1" dense
                  outlined row-height="40" rows="8" single-line></v-textarea>
              </div>
            </v-col>
            ...
            <v-col class="py-0" cols="12" md="12">
              <div class="form-filed-wrapper">
                ...
                <div class="d-inline-block px-2  v-align-top">
                  ...
                  <v-checkbox v-for="item in morning_time_slots" :key="item.id" v-model="form.time_slots_id_list"
                    :label="item.slot_from + ' - ' + item.slot_to" :value="item.id" class="shrink ma-0"
                    hide-details></v-checkbox>
                </div>
                <div class="d-inline-block px-2  v-align-top">
                  <p class="text-1 text-center mb-0">{{ $lang.AFTER_NOON }}</p>
                  <v-checkbox v-for="item in afternoon_time_slots" :key="item.id" v-model="form.time_slots_id_list"
                    :label="item.slot_from + ' - ' + item.slot_to" :value="item.id" class="shrink ma-0"
                    hide-details></v-checkbox>
                </div>
                <div class="d-inline-block px-2 v-align-top ">
                  <p class="text-1  text-center mb-0">{{ $lang.EVENING }}</p>
                  <v-checkbox v-for="item in evening_time_slots" :key="item.id" v-model="form.time_slots_id_list"
                    :label="item.slot_from + ' - ' + item.slot_to" :value="item.id" class="shrink ma-0"
                    hide-details></v-checkbox>
                </div>
              </div>
          ...
          <div class="d-flex  mt-8 mb-4">
            <v-spacer></v-spacer>
            <v-btn color="primary" width="180" @click="Save()" :loading="btn_loader" :disabled="btn_loader">
              {{ $lang.SAVE }}
            </v-btn>
            <v-spacer></v-spacer>
...
export default {
  ...
  created() {
    if (this.$store.getters.getUsertype != this.$keys.ACCOUNT_STUDENT && this.$route.query.student_table_id)
  ...
  methods: {
    ...
    Save() {
      ...
      form.append("guardian_contact_number", self.form.guardian_contact_number);
      ...
      if (this.student_table_id)
        form.append("student_table_id", self.student_table_id);
      ...
      this.request_POST(self, this.$urls.EDIT_DETAIL, form, successHandler, null, null, finallyHandler)
    },
