...
<template>
  <div>
    <div class="mt-5">
      <v-row justify="end">
        <v-col class="" md="6">
          <div class="text-right add-new-btn-text" @click="$router.push({name:'raise_ticket'})">
            {{ $lang.RAISE_TICKET }}
    ...
    <v-row class="mb-0">
      <v-col class="" cols="12" md="6">
        <div class="filter-btn d-inline-block status-text-filter px-4 py-1 mx-2 cursor-pointer"
             v-for="(item,i) in filter_ticket_status_options" :key="i" @click="applyFilter(item.value)"
             :class="filter_ticket_status==item.value?'elevation-5':'elevation-0'">
          {{ item.text }}
          <v-icon v-if="i>0" size="10" style="margin-top:-3px;" class="pl-2" :color="i==1?'#EB5757':'#27AE60'">
    ...
    <div>
      ...
      >
        ...
        <template v-slot:[`item.status`]="{ item }">
          <span :class="'ticket_'+item.status.toLowerCase()" class="px-2">{{ item.status }}</span>
        </template>

        <template v-slot:[`item.action`]="{ item }">
          <div class="d-inline"
               @click="$router.push({name:'support_ticket_details',params:{ticket_table_id:encrypt(item.id)}})">
        ...
        <template v-slot:no-data>
          <EmptyTable
              :message="$lang.EMPTY_RETAILER"
              :type="$keys.EMPTY_TYPE_2"
          />
        </template>
      </v-data-table>
