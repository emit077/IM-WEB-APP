            ...
            <v-simple-table>
              ...
              <tr>
                ...
                <td class="text-right py-1">
                  <small>( - )</small> {{ data.security_deposit.toFixed(2) }}
                </td>
              </tr>
              <tr>
                <td class="py-1">
                  {{ data.transaction_type }} Details <br />
                  <v-text-field
                    v-model="remark"
                    placeholder="Remark"
                    dense
                ...
                <td class="text-right py-1">
                  ...
                  >
                    <template v-slot:append-outer> .00 </template>
                  </v-text-field>
                </td>
              </tr>
              <tr>
              ...
              <tr style="border-top: 1px solid green">
                ...
                <td
                  v-if="data.transaction_type == $keys.DEBIT"
                  class="text-right fw-bold py-1"
                >
                  {{
                    (
                ...
                <td v-else class="text-right fw-bold py-1">
                  {{
                    (
                      parseFloat(data.wallet_amount) -
                      parseFloat(data.security_deposit || 0) +
    ...
    updateWalletBalance() {
      ...
      const successHandler = (response) => {
        if (response.data.success) {
          self.showSnakeBar(
            "success",
            `Wallet successfully ${this.data.transaction_type}ed.`
          );
          this.$emit("yes");
      ...
      this.request_POST(
        self,
        this.$urls.UPDATE_WALLET_AMOUNT,
        form,
        successHandler,
        null,
        null,
