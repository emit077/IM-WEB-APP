        ...
        <v-row class="mb-6">
            <v-col cols="12" md="6">
                <v-text-field v-model="search_query" :placeholder="$lang.PAYOUT_SEARCH" append-icon="mdi-magnify" dense
                    hide-details outlined rounded single-line @keyup="getWalletList"></v-text-field>
            </v-col>
            <v-col class="" cols="3" md="2">
            </v-col>
        </v-row>
        <div>
            <v-data-table :disable-sort="true" :headers="headers" :hide-default-footer="true" :items="tutor_list"
                ...
                <template v-slot:[`item.action`]="{ item }">
                    <!-- <a class="d-inline" :href="$urls[$project.NAME][$project.ENV] + $urls.GET_REGISTRATION_PAYMNET_INVOICE +
            ...
            <v-pagination v-model="page_number" :length="total_page_count" :total-visible="7" class="custom-pagination"
                @input="getWalletList"></v-pagination>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            search_query: "",
            headers: [
                { text: "Payout ID", align: "start", value: "payout_id", width: "120px" },
