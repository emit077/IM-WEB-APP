...
<template>
    <div>
        <v-row class="mb-6">
            <v-col cols="12" md="6">
                <v-text-field v-model="search_query" :placeholder="$lang.PAYOUT_SEARCH" append-icon="mdi-magnify" dense
                    hide-details outlined rounded single-line @keyup="gatRequestList"></v-text-field>
            </v-col>
            <v-col class="" cols="3" md="2">
            </v-col>
        </v-row>
        <div>
            <v-data-table :disable-sort="true" :headers="headers" :hide-default-footer="true" :items="request_list"
                ...
                <template v-slot:[`item.action`]="{ index, item }">
                    <div
                        v-if="item.request_status == $keys.PAYOUT_STATUS_PENDING && [$keys.ACCOUNT_SUPER_ADMIN, $keys.ACCOUNT_ADMIN].includes($store.state.user.account_type)">
                        <v-btn small color="success" class="mx-1"
                            @click="openDialog(item, index, $keys.PAYOUT_STATUS_APPROVED)">{{ $lang.APPROVE }}</v-btn>
                        <v-btn small class="mx-1" color="error"
                            @click="openDialog(item, index, $keys.PAYOUT_STATUS_REJECTED)">{{ $lang.REJECT }}</v-btn>
                    </div>
                </template>
                <!-- action btns -->
            </v-data-table>
            ...
            <v-pagination v-model="page_number" :length="total_page_count" :total-visible="7" class="custom-pagination"
                @input="gatRequestList"></v-pagination>
        </div>
...
export default {
    ...
    methods: {
        ...
        openDialog(item, index, request_status) {
            ...
            this.dialog_data.message = "Are you sure want to " + request_status + " this request. <br>( " + item.request_id + " )";
        ...
        processRequest() {
            ...
            const successHandler = (response) => {
                if (response.data.success) {
                    this.showSnakeBar("success", "Request has been " + this.dialog_data.request_status)
                    this.request_list[this.dialog_data.index].request_status = this.dialog_data.request_status
