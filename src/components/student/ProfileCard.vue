    ...
    <v-row class="mx-0" justify="center">
      <v-col class="text-center" cols="12" md="2" sm="4">
        <v-avatar class="ml-1" size="110">
          <img
              v-if="student_basic_data.gender=='Female'"
              alt="John"
      ...
      <v-col :class="{'pa-0': $vuetify.breakpoint. smAndDown, '': $vuetify.breakpoint. mdAndUp}" class="" cols="12"
        ...
        <div :class="{'fs-16 text-center': $vuetify.breakpoint. smAndDown, 'fs-18': $vuetify.breakpoint. mdAndUp}">
          ...
          <p class="sub-title-2 ma-0  mt-1 small-screen">{{ $lang.STUDENT_ID }} : #<span v-if="student_basic_data"
                                                                                         class="sub-title-2 text-right ma-0">{{
              student_basic_data.student_id
            }}</span>
          </p>
        </div>
      </v-col>
      ...
      <v-col class="large-screen" cols="12" md="2" sm="12">
        <p class="sub-title text-right ma-0">{{ $lang.STUDENT_ID }}</p>
        <p v-if="student_basic_data" class="sub-title-2 text-right ma-0">#{{ student_basic_data.student_id }}</p>
      </v-col>
    </v-row>
    <div
        :class="{'text-center mt-5 mb-n2': $vuetify.breakpoint. smAndDown, 'text-right mb-n4 mt-n5': $vuetify.breakpoint. mdAndUp}">
      <v-btn color="rgba(0, 121, 208, 1)" small @click="$router.push({name:'edit_student_profile',query:{student_table_id:encrypt(student_table_id)}})">
        <img alt="
...
<script>
export default {
  props: ["created", "student_name", "student_basic_data","student_table_id"],
  data() {
    return {}
  },
  methods: {}
