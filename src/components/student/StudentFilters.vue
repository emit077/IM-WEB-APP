          ...
          <v-col md="4" class="border-r pl-4">
            ...
            <div v-for="(item, i) in filter_option" :key="i">
              <div class="py-1 my-1 cursor-pointer" @click="step = i + 1">
                ...
                <span v-if="i == 0" class="" :class="step == i + 1 ? 'fc-primary-light font-weight-bold fs-16' : ''"
                  @click="getClassList">{{ item }}</span>
                <span v-else-if="i == 1" class=""
                  :class="step == i + 1 ? 'fc-primary-light font-weight-bold fs-16' : ''" @click="getCityList">{{ item
                  }}</span>
                <span v-else class="" :class="step == i + 1 ? 'fc-primary-light font-weight-bold fs-16' : ''">{{ item
                  }}</span>
              </div>
            </div>
          </v-col>
              ...
              <v-stepper-items>
                ...
                <v-stepper-content step="1">
                  <v-checkbox v-for="(item, i) in class_list" :key="i" v-model="filter_data.class_id_list"
                    :label="item.class_name" :value="item.id" @change="manageCount" class="ml-2 mt-0 mb-2" dense
                    hide-details>
                      ...
                      <v-menu bottom origin="center center" transition="scale-transition">
                        <template v-slot:activator="{ on, attrs }">
                          <div class="elevation-0" v-bind="attrs" v-on="on">
                            ...
                            <div>
                              <v-text-field v-model="d_form_date" placeholder="From date" dense outlined single-line
                      ...
                      <v-menu bottom origin="center center" transition="scale-transition">
                        <template v-slot:activator="{ on, attrs }">
                          <div class="elevation-0 " v-bind="attrs" v-on="on">
                            ...
                            <div>
                              <v-text-field v-model="d_to_date" placeholder="To date" dense outlined single-line
                                maxlength="2" readonly hide-details></v-text-field>
                            </div>
                          </div>
                        </template>
...
export default {
  ...
  data() {
    return {
      step: 1,
      filter_option: ["Class", "City", "Status", "Profile Status", "Date"],
      class_list: [],
      student_status_options: [{ text: "Active", value: 1 }, { text: "Blocked", value: 0 }],
